...
interface Item {
    ...
    value: string;
    color: string;
    percentage?: string; // Optional to handle missing values
    isIncrease?: boolean;
}
  ...
  return (
    <div className="container mx-auto py-2 px-6 md:px-4">
      ...
      <div className="mt-6 grid grid-cols-4 gap-4">
        {metricsData.map((metric: Metric, index: number) => (
            ...
            <div key={index} className="shadow-sm rounded-lg p-4" style={{ backgroundColor: item.color }}>
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <p className="font-medium text-xs text-[#f5f5f5]">{item.percentage ?? "N/A"}</p>
                </div>
              </div>
